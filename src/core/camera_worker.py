...
class CameraWorker(QThread):
    ...
    def __init__(
        ...
        logger.info(f"CameraWorker 초기화: {camera_width}x{camera_height} @ {camera_fps} FPS")
    ...
    def process_frame(self, frame: np.ndarray) -> dict:
        ...
        # 2. 지표 계산
        try:
            indicators = self.indicator_calculator.calculate_all_indicators(landmarks)
        except Exception as e:
        ...
        try:
            ...
            posture_type = judgment_result.get('posture_type', 'normal')
            probability = judgment_result.get('probability', 0.0)
            
            # 프레임 누적 (연속 판정용)
            self.judgment_engine.accumulate_frame(posture_type)
    ...
    def _annotate_frame(
        ...
        # 프레임 테두리
        cv2.rectangle(annotated, (0, 0), (annotated.shape[1]-1, annotated.shape[0]-1), color, 3)
        ...
        if landmarks.pose_landmarks and len(landmarks.pose_landmarks) > 0:
            nose = landmarks.pose_landmarks[0]
            if nose.confidence > 0.5:
                x = int(nose.x * annotated.shape[1])
                y = int(nose.y * annotated.shape[0])
