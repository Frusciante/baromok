...
#         # 임계값 이상인 자세들만 고려
#         above_threshold = {k: v for k, v in likelihoods.items() if v > 0.3}
#         candidates = {
...
#                 cheek_change / cheek_threshold, min_val=0, max_val=2.0
#             )
#             ratio_score = self._normalize_score(
#                 ratio_change / ratio_threshold, min_val=0, max_val=2.0
#             )
...
#             # 점수 계산
#             cheek_score = self._normalize_score(
#                 abs(cheek_change) / cheek_threshold, min_val=0, max_val=2.0
#             )
...
#                 shoulder_tilt / threshold, min_val=0, max_val=2.0
#             )
...
#             tilt_delta = abs(indicators.shoulder_tilt_deg - baseline_tilt)
#             triggered = tilt_delta > threshold

#             neck_offset_change = self.baseline_manager.calculate_change_percentage(
#                 indicators.neck_offset, "neck_offset"
#             )

#             triggered = tilt_delta > threshold and neck_offset_change > 10
...
#             triggered = eye_triggered or shoulder_triggered
#             likelihood = 0.5 * eye_score + 0.2 * shoulder_score + 0.3 * hand_score
...
#     config: ConfigManager, baseline_manager: BaselineManager
# ) -> JudgmentEngine:
#     """판정 엔진 생성 (팩토리 함수)"""
...
class JudgmentEngine:
    ...
    def judge_single_frame(
        ...
        # 기존 방식처럼 likelihood > 0.3만으로 후보에 넣으면,
        # 실제 조건이 발동되지 않았는데도 턱 괸 자세/다리 꼰 자세가 화면에 표시될 수 있다.
        candidates = {
            PostureType.FORWARD_HEAD.value: forward_head_result,
            PostureType.RECLINE.value: recline_result,
            PostureType.CROSSED_LEG.value: crossed_leg_result,
    ...
    def _judge_crossed_leg(self, indicators: PostureIndicators) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(
                f"[다리꼰자세] current_shoulder_tilt={indicators.shoulder_tilt_deg:.1f}°, "
