...
3. **RANSAC Linear Model 헬퍼 구현 (`src/utils/helpers.py`)**
   - RANSAC 적합 관리를 위한 `RansacLinearModel` 클래스를 신설했습니다.
   - 의존성 지연 로드(lazy import)를 적용해 파일 시작 시 런타임 오버헤드를 막고, `scikit-learn`의 `PolynomialFeatures(degree=1)`와 `RANSACRegressor`를 파이프라인으로 묶어 1차 선형 예측 모델의 생성과 결과값 반환을 처리합니다.
