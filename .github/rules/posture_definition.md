...
### 다리 꼰 자세 (비대칭 추정)
- **의미**: 하체 비대칭이 상체 어깨 높이 변화로 나타난 상태 추정
- **관찰**: 어깨 기울기 및 목 정렬 지표의 변화
- **판정**: 임계값 및 지속 시간은 <a href="./operation/posture_types.crossed_leg_estimated">posture_types.crossed_leg_estimated</a>를 참조하십시오.
...
| :--- | :--- | :--- | :--- |
| **기댄 자세** | 비율 감소, 거리 변화 | JSON 참조 | JSON 참조 |
| **거북목** | 비율 증가, 거리 변화 | JSON 참조 | JSON 참조 |
| **다리 꼰 자세** | 어깨 기울기, 비대칭 | JSON 참조 | JSON 참조 |
| **턱 괸 자세** | 기울기, 가림, 근접 | JSON 참조 | JSON 참조 |
