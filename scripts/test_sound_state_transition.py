...
def test_state_transition_and_sound():
    ...
    try:
        ...
        def handle_state_transition(event: StateTransitionEvent):
            ...
            logger.info(f"상태 전이 콜백: {event.from_state.value} -> {event.to_state.value}")
            ...
            if event.to_state.value == "bad_posture":
                logger.info(f"  [조건 충족] sound_enabled={settings.sound_enabled} and to_state=='bad_posture'")
                if settings.sound_enabled:
                    logger.info(f"  → _show_alert_popup에서 _play_alert_sound_async() 호출 예정")
                    # 실제 재생
